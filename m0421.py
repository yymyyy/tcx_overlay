...
# 加载数据
data = pd.read_csv('c:\\Users\\yao\\Desktop\\2024-4-21nantong_final_csv.csv')
#data = pd.read_csv('c:\\Users\\yao\\Desktop\\2024-4-21nantong_final_csv - clip.csv')
...
def seconds_to_time(seconds):
    ...
    minutes, seconds = divmod(remainder, 60)
    # 格式化为字符串时间格式
    time_str = "{:d}:{:02d}:{:02d}".format(int(hours), int(minutes), int(seconds))
    return time_str
...
def plot_map(data,i):
    ...
    axes[4].clear()
    axes[4].scatter(range(len(data)),data['speed2'], c='#55CC66', s=1)  # 6Db3fB blue
...
def update_time(i):
    ...
    lapstext = str(data['laps'][i])
    avgspeed = speed_to_pace(distance / time_to_seconds(time_pass)) #平均配速
    guessfinish = seconds_to_time(42195/(distance / time_to_seconds(time_pass)))
    j=0
    plot_map(data,i)
    for ax in axes:
        if j==0:
            ax.clear()  # 清空ax
            ax.text(0, 0.5, f"配速: {speed} DR {speed2}\n心率: {heart_rate}\n步频: {cadence} DR {cadence2}\n平均配速: {avgspeed} 预计完赛 {guessfinish}", fontsize=24, ha='left', va='center', color='#55CC66', transform=ax.transAxes)
        if j==1:
            ax.clear()  # 清空ax
            ...
            if float(data['speed2'][i])<3.52:
                ...
                turtle_index = (i) % 2 
            # 设置乌龟图像的位置（左上角）
            turtle_img_objs[0].set_offset((380, 70))
            turtle_img_objs[1].set_offset((380, 70))
            turtle_img_objs[2].set_offset((380, 70))
            turtle_img_objs[3].set_offset((380, 70))
        ...
        if j==6:
            ax.clear()  # 清空ax
            #print("laps=",type(lapstext),lapstext)
            if len(lapstext)>3:
            #lapstext="Start"
                ax.text(0, 0.5, f"佳明255手表，{lapstext}", fontsize=48, ha='left', va='center', color='#55CC66', transform=ax.transAxes)
